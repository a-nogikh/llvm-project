...
; CHECK: returns_noalias_field attribute requires a valid integer value
define {ptr} @test_bad_attr_val() #1 {
  ret {ptr} zeroinitializer
}

; CHECK: returns_noalias_field index is out of bounds for return type
define {ptr} @test_idx_out_of_bounds() #2 {
  ret {ptr} zeroinitializer
}

; CHECK: returns_noalias_field index must refer to a pointer type field
define {i64} @test_field_not_ptr() #3 {
  ret {i64} zeroinitializer
}

attributes #0 = { "returns_noalias_field"="0" }
attributes #1 = { "returns_noalias_field"="abc" }
attributes #2 = { "returns_noalias_field"="1" }
attributes #3 = { "returns_noalias_field"="0" }